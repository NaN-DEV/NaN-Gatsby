  ...
  componentDidMount() {
    ...
    content.category.forEach(category => {
      ...
      content.articles.forEach(article => {
        return article.skils.filter(post => post.id === category.id).length > 0 && ItemInCategory.push(article);
      });
            ...
            <Left theme={settings}>
              <Sticky id={`${id}-sticky`} theme={settings} offsetTop={120}>
                <Category id={`${id}-sticky`} theme={settings}>
                  <Title theme={settings}>Profesja</Title>
                  <List type="vertical" newClass="listCategory">
                    ...
                    >
                      Wszystkie ( {content.articles.length} )
                    </Button>
                      ...
                      index < parameters.skip + parameters.limit && (
                        <BoxArticle theme={settings} id={`${article.id}-BoxArticle-id`} key={`${article.id}-BoxArticle-key`}>
                          <Article
                            type="article"
                            image={article.image}
                            title={`${article.name} ${article.surname}`}
                            id={`${article.id}-id`}
                            key={`${article.id}-key`}
                            to={`/expert/${article.slug}`}
                          />
                        </BoxArticle>
                      )
                    );
