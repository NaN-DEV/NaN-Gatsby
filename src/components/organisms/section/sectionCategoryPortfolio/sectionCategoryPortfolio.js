...
import Article from '../../../molecules/article/articleShort/articleShort';
            ...
            <Sticky id={`${parameters.id}-sticky`} offsetTop={60}>
              <Category id={`${parameters.id}-sticky`} theme={{ settings }}>
                <List type="vertical" parameters={{}}>
                  <Button type="linkIn" content={{ to: '/p/' }} parameters={{ className: parameters.id === 'all' && 'desactive' }}>
                    wszystko ( {content.articles.length} )
                  </Button>
                  {content.category.map((item, i) => {
                    return item.tag.title === 'ALL' ? null : (
                      <Button type="linkIn" content={{ to: `/p/${item.slug}` }} parameters={{ className: parameters.id === item.id && 'desactive' }}>
                  ...
                  return (
                    ...
                    index < parameters.skip + parameters.limit && (
                      <BoxArticle theme={{ settings }}>
                        <Article type="articleShort" content={{ title: article.title }} parameters={{ to: `/p/p/${article.slug}` }} />
                      </BoxArticle>
                    )
                  );
                })
